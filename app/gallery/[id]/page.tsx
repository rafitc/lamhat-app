...
const mockGalleries = {
  ...
  2: {
    ...
    photos: [
      ...
      {
        id: 2,
        url: "https://picsum.photos/400/303",
        caption: "Sunset",
      },
    ],
  },
...
export default function GalleryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [gallery, setGallery] = useState(mockGalleries[galleryId as keyof typeof mockGalleries]);
  ...
  useEffect(() => {
    // In a real app, you would fetch the gallery data from an API
    if (!gallery) {
      router.push("/home");
    }
  }, [gallery, router]);
  ...
  const handleTitleBlur = () => {
    if (gallery.name.trim() === "") {
      setGallery({ ...gallery, name: "My Gallery" });
    }
  ...
  const confirmDelete = () => {
    if (photoToDelete) {
      setGallery({
        ...gallery,
        photos: gallery.photos.filter((photo) => photo.id !== photoToDelete),
      });
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || gallery.photos.length >= 20) return;
    ...
    setGallery({
      ...gallery,
      photos: [...gallery.photos, ...newPhotos].slice(0, 20),
    });
  };

  const handleSaveGallery = () => {
    // Mock API call to save gallery
    console.log("Saving gallery:", gallery);
  };

  const togglePublish = () => {
    setGallery({ ...gallery, isPublished: !gallery.isPublished });
  };
            ...
            >
              <div className="flex flex-col items-center justify-center h-full min-h-[200px]">
                ...
                <p className="text-sm text-gray-500">
                  {20 - gallery.photos.length} slots remaining
                </p>
              </div>
              <input
                ref={fileInputRef}
